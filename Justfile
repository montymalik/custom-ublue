set dotenv-filename := "custom-ublue.env"
set dotenv-load
...
# prev_ref: a published image whose layer plan the new build should follow, so
# that unchanged content lands in identical layers and updates stay small.
ostree-rechunk $target_image=image_name $tag=default_tag $prev_ref=("docker://ghcr.io/" + repo_organization + "/" + image_name + ":" + default_tag):
    #!/usr/bin/env bash
    ...
    # Only reuse the previous layer plan if that image actually exists
    # (first build of a fork, private registry, offline).
    PREV_ARGS=()
    if [[ -n "${prev_ref}" ]] && podman manifest inspect "${prev_ref#docker://}" >/dev/null 2>&1; then
        PREV_ARGS+=("--previous-build" "${prev_ref}")
    fi

    # The chunker writes a fresh image config, so labels set by `just build`
    # (and the Containerfile) would be lost. --from carries the config over and
    # the explicit --label list makes sure every label survives regardless.
    LABEL_ARGS=()
    while IFS= read -r kv; do
        LABEL_ARGS+=("--label" "${kv}")
    done < <(podman inspect "${target_image}:${tag}" --format '{{ '{{json .Labels}}' }}' | jq -r 'to_entries[] | "\(.key)=\(.value)"')
    ...
    podman run --rm --pull=never --privileged \
      ...
      --format-version=2 \
      --bootc \
      --from "[overlay@/run/host-container-storage+/run/rpm-ostree-storage]localhost/${target_image}:${tag}" \
      "${PREV_ARGS[@]}" \
      "${LABEL_ARGS[@]}" \
      --rootfs /rpm-ostree \
...
generate-build-tags $target_image=image_name $tag=default_tag:
    ...
    # Tag scheme: latest, latest.YYYYMMDD, YYYYMMDD (matches what has been
    # published so far, so existing bootc/rpm-ostree references keep working)
    DATE=$(date +%Y%m%d)
    BUILD_TAGS=()
    BUILD_TAGS+=("${tag}")
    BUILD_TAGS+=("${tag}.${DATE}")
